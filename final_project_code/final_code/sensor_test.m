...
if (clientID > -1)
    ...
    for i = 1:5000
        ...
        if ~isempty(refPointPosition)
            ...
            % 라이다 데이터 가져오기
            [~, detectionState, detectedPoint, ~, ~] = vrep.simxReadProximitySensor(clientID, lidar, vrep.simx_opmode_blocking);
            
            % 라이다로 감지된 점들을 맵에 표시
            if detectionState
                % 점들을 맵 좌표로 변환하여 표시
                for i = 1:numel(detectedPoint)/3
                    x = round((robotPosition(1) + detectedPoint(1 + (i-1)*3) + mapSize/2) / mapResolution);
                    y = round((robotPosition(2) + detectedPoint(2 + (i-1)*3) + mapSize/2) / mapResolution);
                    map(y, x) = 1; % 장애물로 표시
                end
            end
